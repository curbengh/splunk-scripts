...
def main(input: str = "", cloud: bool = False):
    ...
    while not path.isfile(app_gz):
        print(
            "Download the latest [Splunk Add-on for VMware ESXi Logs]"
            "(https://splunkbase.splunk.com/app/5603) with a Splunk.com account."
        )
    ...
    new_gz = path.join(app_dir, f"Splunk_TA_esxilogs_{version}.tgz")
...
if __name__ == "__main__":
    files = Path(getcwd()).glob("splunk-add-on-for-vmware-esxi-logs_*.tgz")
    app_paths = []
    for i in files:
        app_paths.append(i)
